...
#GET ALL TRIPS
@bpCtrip.route('/trip', methods=['GET'])  # type: ignore
def all_trips():
    trips= CreateTrip.query.all()
    trips= list(map(lambda user: user.serialize(), trips))
    return jsonify(trips), 200

# #GET TRIP BY ID
@bpCtrip.route('/users/<int:id>', methods= ['GET'])  # type: ignore
def trip_by_id(id):
    trip= CreateTrip.query.get(id)
    return jsonify(trip.serialize()), 200

#DELETE TRIP BY ID
@bpCtrip.route('/trip/<int:id>/delete', methods=['POST'])
def delete_trip(id):
    trip= CreateTrip.query.get(id)
    trip.delete()
